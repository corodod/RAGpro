...
Твоя задача: разложить сложный вопрос на небольшой граф под-вопросов (DAG).

Верни ТОЛЬКО строки (по одной на строку), без объяснений.

Формат строки:
Q<n>: <текст под-вопроса> | dep=Qk[,Qm...] | slot=<name>

Правила:
- dep отсутствует у корневых вопросов (которые можно выполнять сразу).
- slot=<name> указывай только если ответ нужен как переменная для других вопросов.
- Если под-вопрос использует переменную, вставь плейсхолдер {slot} в текст.
- Кол-во под-вопросов: 1..5. deps на вопрос: 0..2.
- Думай логически: если нужен A→X→B, то сначала получаем X, потом используем X.
""".strip()
