...
- Retrieval на каждом hop: выполняется поиск по текущему запросу через base_retriever, из документов извлекаются факты.
- Answer probe: LLM (Qwen/Qwen2.5-3B-Instruct или TinyLlama/TinyLlama-1.1B-Chat-v1.0) пытается сгенерировать короткий ответ на основе последних фактов.
- Проверка стабилизации: текущий ответ сравнивается с предыдущим через cosine similarity эмбеддингов dense retrieval; при схожести ≥ 0.90 ответ считается стабильным.
- Planner: MultiHopPlanner анализирует накопленные факты и предыдущие запросы, формирует уточняющий NEXT_QUERY или отдаёт команду STOP.
- Цикл повторяется до достижения max_hops или стабилизации ответа.
- Финальный rerank: после завершения loop документы при необходимости переупорядочиваются Cross-Encoder’ом (ce_score).
---
